...
sub cleanup {
  ...
  $text =~ s!raw\.githubusercontent\.com/check-spelling/check-spelling!raw\.githubusercontent\.com/CHECK-SPELLING/CHECK-SPELLING!g;
  ...
  $text =~ s/$sandbox/WORKSPACE/g;
  $text =~ s!/tmp/check-spelling!TEMP_DIRECTORY!g;
  $text =~ s/$working_directory/ENGINE/g;
  $text =~ s!$github_repository!GITHUB_REPOSITORY_OWNER/GITHUB_REPOSITORY_NAME!g;
  ...
  if (defined $internal_state_directory) {
    ...
    $text =~ s/$internal_state_directory/INTERNAL_STATE_DIRECTORY/g;
    if ($text =~ m{git am <<'\@\@\@\@([0-9a-f]{40}--\d+)'}) {
      my $am_marker = $1;
      $text =~ s/$am_marker/AM_MARKER/g;
    ...
    if ($text =~ m{create mode 100644 .github/actions/spelling/expect/([0-9a-f]{3})([0-9a-f]{37})\.txt}) {
      ...
      $text =~ s/$sha_prefix$sha_suffix/EXPECT_FULL_SHA/g;
